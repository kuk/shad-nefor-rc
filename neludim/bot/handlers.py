...
def start_text(context):
    return f'''Бот @neforshad_random_coffee_bot организует random coffee для неформала ШАДа.
...
Автор проекта Наташа https://github.com/natasha - библиотеки для русскоязычного NLP. Работаю в своей Лабе https://lab.alexkuk.ru, адаптирую Наташу под задачи клиентов

Живу в Москве в Крылатском. Бегаю. Занимаюсь древнерусской каллиграфией. Вожу экскурсии для друзей, рассказываю про сталинский генплан, импрессионистов и супрематистов."

"Я закончил ФКН ВШЭ и ШАД в 2021 году. За это время успел постажироваться в Алисе в качестве аналитика и ML-разработчика — занимался NLP и распознаванием речи. Затем поработал 1.5 года в команде Зелибобы (YaLM) [https://github.com/yandex/YaLM-100B; https://yandex.ru/lab/yalm] в качестве ML-разработчика.
Недавно переехал в Амстердам заниматься машинным переводом в Booking.com.
Увлекаюсь стартапами, хочется влететь в эту историю попозже. Занимаюсь спортом, люблю читать научную фантастику)"'''
...
HELP_TEXT = '''Бот @neforshad_random_coffee_bot организует random coffee для неформала ШАДа.

<b>Как это работает?</>
- Участник чата "Неформал ШАД" запускает бота, заполняет анкету. Бот случайно объединяет участников в пары.
